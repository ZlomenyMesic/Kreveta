...
internal static class FutilityPruning {
    ...
    private const int PVMargin                 = 16;

    // try futility pruning
    internal static bool TryPrune([In, ReadOnly(true)] in Board board, int depth, Color col, short staticEval, bool improving, bool isPV, Window window) {
        ...
        int margin = (FutilityMarginBase + pawnCorrection + _improving + _isPV + FutilityMarginMultiplier * depth) * (col == Color.WHITE ? 1 : -1);
